...
def call_deepseek(prompt: str, stock_code: str = "") -> Optional[str]:
    """
    使用 DeepSeek 分析股票（独立函数）
    """
    api_key = os.environ.get('DEEPSEEK_API_KEY')
    if not api_key:
        logger.error("錯誤: 沒有設置 DEEPSEEK_API_KEY")
        return None
    ...
    try:
        logger.info(f"正在使用 DeepSeek 分析 {stock_code}...")
        response = requests.post(url, headers=headers, json=data, timeout=60)
        if response.status_code == 200:
            result = response.json()
            return result['choices'][0]['message']['content']
        else:
            logger.error(f"DeepSeek API錯誤: {response.status_code} - {response.text}")
            return None
    except Exception as e:
        logger.error(f"DeepSeek調用失敗: {e}")
        return None
...
```json
{
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        优先级：DeepSeek > Gemini > OpenAI 兼容 API
        ...
        self._use_deepseek = False  # 是否使用 DeepSeek
        self._deepseek_api_key = None  # DeepSeek API Key
        
        # 優先檢查 DeepSeek API Key
        self._deepseek_api_key = os.getenv('DEEPSEEK_API_KEY')
        if self._deepseek_api_key and not self._deepseek_api_key.startswith('your_') and len(self._deepseek_api_key) > 10:
            logger.info("检测到 DeepSeek API Key，将优先使用 DeepSeek")
            self._use_deepseek = True
            self._current_model_name = "deepseek-chat"
            return  # 直接返回，不初始化 Gemini
        
        # 檢查 Gemini API Key 是否有效（过滤占位符）
    ...
    def is_available(self) -> bool:
        """检查分析器是否可用"""
        return self._model is not None or self._openai_client is not None or self._use_deepseek
    ...
    def analyze(
        ...
        if not self.is_available():
            return AnalysisResult(
                ...
                risk_warning='请配置 DeepSeek 或 Gemini API Key 后重试',
                success=False,
                error_message='未配置任何 AI API Key',
            )
        
        # 如果使用 DeepSeek，直接調用
        if self._use_deepseek:
            logger.info(f"使用 DeepSeek 分析 {name}({code})")
            
            # 格式化提示詞（重用原來的 _format_prompt 方法）
            prompt = self._format_prompt(context, name, news_context)
            
            # 調用 DeepSeek
            deepseek_result = call_deepseek(prompt, code)
            
            if deepseek_result:
                # 解析返回結果
                return self._parse_response(deepseek_result, code, name)
            else:
                # DeepSeek 失敗，返回錯誤結果
                return AnalysisResult(
                    code=code,
                    ...
                    operation_advice='持有',
                    confidence_level='低',
                    analysis_summary='DeepSeek 分析失敗',
                    risk_warning='請檢查 API Key 和網絡連接',
                    success=False,
